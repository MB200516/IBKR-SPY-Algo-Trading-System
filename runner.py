...
class AlgoRunner:
    ...
    def _connect(self) -> bool:
        ...
        try:
            ...
            # Must be called immediately after connection, before any data requests.
            # Types: 1 = Live, 2 = Frozen, 3 = Delayed (free, no subscription needed),
            #        4 = Delayed-Frozen
            if ib_cfg.use_delayed_data:
                self._ib.reqMarketDataType(3)
                self._log.info(
                    "Market data mode: DELAYED (type 3) — "
                    "data is 15–20 minutes behind real-time."
                )
                self._log.warning(
                    "TESTING MODE ONLY: Delayed market data is active. "
                    "Do NOT use this configuration for live trading decisions. "
                    "Enable a live market data subscription and set "
                    "use_delayed_data = False in config.py before going live."
                )
            else:
                self._ib.reqMarketDataType(1)
                self._log.info("Market data mode: LIVE (type 1).")
    ...
    def start(self):
        ...
        self._log.info(
            f"  Market Data: {'DELAYED (testing only)' if self.cfg.ibkr.use_delayed_data else 'LIVE'}"
        )
        self._log.info("=" * 55)
